...
## Résumé
Dans ce deuxième article sur l'écriture, je montre comment mettre en oeuvre les techniques décrites dans le premier article. Pour cela, je présente les outils que j'ai choisi et comment les utiliser pour démarrer un projet d'écriture. 
...
Maintenant que nous avons défini une structure narrative qui peut s'adapter à une vingtaine de chapitres, et que nous avons compris l'importance du dynamisme des personnages pour chaque scène, il est temps de démarrer un projet qui servira d'exemple. 
Nous avons donc besoin de choisir des outils qui nous permettront de : 
...
Je précise que les deux premiers sont assez proches et qu'il serait tout-à-fait possible d'en utiliser qu'un seul. Toutefois, il y a des différences qui me forcent à les utiliser tous les deux en parallèle, même si cela entraîne parfois un peu de redondance...
En ce qui concerne le traitement de texte (TTX), j'ai choisi Google Docs mais évidemment c'es un choix qui n'engage que moi. Tout autre outil similaire ferait l'affaire.

---

## Création d'un projet
Nous utiliserons Airtable pour modéliser les scènes et les chapitres. L'intérêt de Airtable est, outre le fait qu'il est disponible dans une version gratuite, qu'il propose des vues sous forme de cartes et de tableaux, ainsi qu'un Kanban pour visualiser le changement de statut et l'avancement.
...
Voici comment configurer une vue Kanban dans la table "Scènes" étape par étape, en optimisant pour le projet de 20 chapitres et 300 pages.

---

### Étape 1 : Préparer la table "Scènes" dans Airtable
Avant de configurer la vue Kanban, nous allons créer une table "Scènes" contenant les champs essentiels pour gérer un roman. C'est un peu comme une feuille de tableur. Voici une structure suggérée :
...
- **Astuce :** Ajoute ces champs via le bouton "+" dans Airtable et remplis quelques scènes pour tester.
...
Exemple de carte :
```
Poursuite ruelle sombre
Résumé : Alex fuit Léo dans une ruelle, trouve une arme.
Personnages : Alex, Léo, Sara
Lien : [Google Docs]
Mots : 800
```
...
### Étape 5 : Utiliser le Kanban avec Google Docs
- **Workflow:**
  1. Créer une scène dans Airtable (ex. : "Poursuite ruelle sombre", Statut = "À écrire").
  2. Ouvrir un nouveau Google Doc, écrire la scène, et copier son URL dans le champ "Lien Google Docs".
  3. Déplacer la carte dans "En cours" pendant l’écriture, puis "Terminée" une fois finie.
- **Synchronisation:** Le Kanban donne une vue d’ensemble, et Google Docs reste l'unique espace d’écriture.
...
### Exemple final de Kanban (Statut)
```
À écrire       | En cours          | Terminée
---------------|-------------------|----------------
Scène 1 Chap 1 | Poursuite ruelle  | Scène 3 Chap 2
Scène 2 Chap 1 |                   | 
```
...
---------------|-------------------|----------------
Scène 1        | Scène 3           | Poursuite ruelle
Scène 2        |                   | 
```
...
### Pour résumer
Cette logique est non seulement correcte, mais elle donne une structure organique au projet d'écriture. Chaque scène devient un moteur narratif, chaque chapitre un jalon, et le schéma une toile de fond qui lie le tout. C’est une bonne recette pour un roman captivant !
